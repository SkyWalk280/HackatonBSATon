...
function ReactionContent() {
    ...
    delayTimeout.current = setTimeout(() => {
      ...
      missTimeout.current = setTimeout(() => {
        ...
        setReactionTimes(prev => {
          ...
          if (next >= ROUNDS) setTimeout(() => setPhase("finished"), 900);
          else { setRound(next); setTimeout(() => startRoundRef.current(next), 900); }
          return newTimes;
        });
      }, MISS_TIMEOUT);
    }, delay);
  ...
  const startGame = useCallback(() => {
    ...
    const interval = setInterval(() => {
      c -= 1;
      setCountdown(c);
      if (c === 0) { clearInterval(interval); startRoundRef.current(0); }
    }, 1000);
  }, []);
  ...
  const handleHit = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    haptic.impact(reactionMs < 300 ? "heavy" : "medium");
    ...
    setReactionTimes(prev => {
      ...
      const next = round + 1;
      if (next >= ROUNDS) setTimeout(() => setPhase("finished"), 600);
      else { setRound(next); setTimeout(() => startRoundRef.current(next), 600); }
      return newTimes;
    });
  ...
  useEffect(() => {
    if (phase === "finished" && !submitted && reactionTimes.length > 0) submitScore(reactionTimes);
      ...
      <div style={s.page}>
        ...
        <div style={s.card}>
          ...
          <div style={s.scoreRow}>
            <div style={s.scoreBox}><div style={s.scoreLabel}>Your avg</div><div style={s.scoreNum}>{toMs(myInv)}<span style={{ fontSize:12 }}>ms</span></div></div>
            <div style={{ fontSize:20, color:"var(--text-muted)", alignSelf:"center" }}>vs</div>
            <div style={s.scoreBox}><div style={s.scoreLabel}>Opponent</div><div style={s.scoreNum}>{toMs(oppInv)}<span style={{ fontSize:12 }}>ms</span></div></div>
          </div>
          ...
          {isWinner && (
            <button style={{ ...s.btn, background:"var(--bg-card)", border:"1px solid var(--border-active)", color:"var(--ton-blue)" }} onClick={() => {
              const msg = `I just won ${prizeDisplay} BSA USD in Reaction Time! ⚡ Challenge me!`;
          ...
          <button style={s.btn} onClick={() => router.push(`/?presetMode=reaction&presetBet=${matchResult.betAmount ?? 0.01}`)}>⚔️ Rematch</button>
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        <div style={{ display:"flex", justifyContent:"space-between", padding:"0 4px 12px" }}>
          <div>
            ...
            <div style={{ fontSize:20, fontWeight:700, fontFamily:"var(--font-mono)", color:"var(--text-primary)" }}>{round + 1} / {ROUNDS}</div>
          </div>
          <div style={{ textAlign:"right" as const }}>
            <div style={{ fontSize:10, color:"var(--text-muted)", textTransform:"uppercase" as const }}>Avg</div>
            <div style={{ fontSize:20, fontWeight:700, fontFamily:"var(--font-mono)", color:"var(--ton-blue)" }}>{avgMs > 0 ? `${avgMs}ms` : "—"}</div>
          </div>
        </div>
        <div onClick={handleMiss} onTouchStart={handleMiss} style={{
          position:"relative" as const, width:"100%", height:containerH,
          background: phase === "too_early" ? "rgba(255,50,50,0.08)" : phase === "missed" ? "rgba(255,100,0,0.08)" : phase === "correct" ? "rgba(57,198,136,0.05)" : "var(--bg-card)",
          border: `1px solid ${phase === "too_early" ? "rgba(255,50,50,0.4)" : phase === "missed" ? "rgba(255,100,0,0.4)" : phase === "correct" ? "rgba(57,198,136,0.3)" : "var(--border)"}`,
          borderRadius:14, overflow:"hidden", cursor:"crosshair", userSelect:"none" as const, transition:"background 0.15s, border 0.15s",
        }}>
          <div style={{ position:"absolute" as const, top:"50%", left:"50%", transform:"translate(-50%,-50%)", textAlign:"center" as const, pointerEvents:"none" as const }}>
            {phase === "get_ready" && <div><div style={{ fontSize:36 }}>👀</div><div style={{ fontSize:14, color:"var(--text-secondary)", fontWeight:600, marginTop:4 }}>Wait for the target...</div></div>}
            {phase === "too_early" && <div><div style={{ fontSize:36 }}>⚠️</div><div style={{ fontSize:14, color:"var(--error)", fontWeight:600, marginTop:4 }}>Too early! Restarting...</div></div>}
            {phase === "missed" && <div><div style={{ fontSize:36 }}>💨</div><div style={{ fontSize:14, color:"var(--warning)", fontWeight:600, marginTop:4 }}>Missed! +{MISS_PENALTY}ms</div></div>}
            {phase === "correct" && lastReaction !== null && <div><div style={{ fontSize:36 }}>⚡</div><div style={{ fontSize:28, fontWeight:700, fontFamily:"var(--font-mono)", color:"var(--success)", marginTop:4 }}>{lastReaction}ms</div></div>}
          </div>
          {targetVisible && (
            <div onClick={handleHit} onTouchStart={handleHit} style={{
              position:"absolute" as const, left:targetX, top:targetY, width:TARGET_SIZE, height:TARGET_SIZE,
              borderRadius:"50%", background:"radial-gradient(circle at 35% 35%, #5bf, var(--ton-blue))",
              boxShadow:"0 0 24px rgba(0,136,255,0.7), 0 0 48px rgba(0,136,255,0.3)",
              display:"flex", alignItems:"center", justifyContent:"center",
              animation:"pulse-target 0.25s ease-out", cursor:"pointer",
            }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" as const, padding:"10px 4px 0" }}>
            {reactionTimes.map((t, i) => (
              <div key={i} style={{
                ...
                background: t >= MISS_PENALTY ? "rgba(255,100,0,0.15)" : t < 250 ? "rgba(57,198,136,0.15)" : t < 450 ? "rgba(255,169,64,0.15)" : "rgba(255,92,50,0.15)",
                border: `1px solid ${t >= MISS_PENALTY ? "rgba(255,100,0,0.5)" : t < 250 ? "rgba(57,198,136,0.4)" : t < 450 ? "rgba(255,169,64,0.4)" : "rgba(255,92,50,0.4)"}`,
...
const s: Record<string, React.CSSProperties> = {
  ...
  card: { background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:16, padding:24, width:"100%", maxWidth:360, display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", gap:10 },
  ...
  btn: { width:"100%", padding:"13px 16px", background:"var(--ton-blue)", border:"none", borderRadius:10, color:"#fff", fontSize:15, fontWeight:600, cursor:"pointer" },
