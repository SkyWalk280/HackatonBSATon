...
export default function Page() {
  ...
  const joinPayment = usePayment({
    ...
    onSuccess: useCallback(async (_data: any) => {
      try {
        ...
        // role=player2 so game knows who you are
        router.push(`/game?matchId=${match.matchId}&playerAddress=${wallet?.account?.address}&seed=${match.seed}&role=player2`);
      } catch (err: any) {
  ...
  // Player 1 polls waiting room until opponent joins
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === "playing") {
          ...
          // role=player1
          router.push(`/game?matchId=${matchId}&playerAddress=${wallet?.account?.address}&seed=${matchSeed}&role=player1`);
        }
      } catch {}
    }, 2000);
      ...
      <div style={s.body}>
          ...
          <>
            <div style={s.card}>
              ...
              {[
                ...
                ["2","Stack blocks","Same seed = same game for both"],
        ...
        {screen === "waiting" && (
          <div style={s.card}>
            ...
            <p style={s.cardDesc}>Share this ID with your opponent.</p>
        ...
        {screen === "joining" && (
          <div style={s.card}>
            ...
            <p style={s.cardDesc}>Enter the match ID from your opponent, then pay the entry fee.</p>
...
const s: Record<string, React.CSSProperties> = {
  ...
  errBox: { background:"rgba(255,92,92,0.08)", border:"1px solid rgba(255,92,92,0.25)", borderRadius:8, padding:"8px 12px", fontSize:12, color:"var(--error)", display:"flex", alignItems:"center" },
